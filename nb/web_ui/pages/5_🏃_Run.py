...
def termination():
    ...
    for fname, label in [(log_file, "当前日志"), (old_log, "上次日志")]:
        try:
            ...
            if content.strip():
                st.download_button(
                    f"📥 下载{label}", data=content,
                    file_name=f"nb_{label}.txt", key=f"dl_{label}"
                )
...
if check_password(st):
    with st.expander("运行配置"):
        CONFIG.show_forwarded_from = st.checkbox(
            '显示"转发自"', value=CONFIG.show_forwarded_from
        )
        ...
        if mode == "past":
            ...
            if CONFIG.login.user_type == 0:
                ...
                st.warning(
                    "past 模式仅支持 User 账号，"
                    "Telegram 不允许 Bot 遍历聊天历史。"
                )
            CONFIG.past.delay = st.slider(
                "延迟（秒）", 0, 100, value=CONFIG.past.delay
            )
        else:
            CONFIG.mode = 0
            CONFIG.live.delete_sync = st.checkbox(
                "同步删除消息", value=CONFIG.live.delete_sync
            )
    ...
    if os.path.exists(log_file):
        lines = st.slider(
            "显示日志行数", min_value=50, max_value=2000,
